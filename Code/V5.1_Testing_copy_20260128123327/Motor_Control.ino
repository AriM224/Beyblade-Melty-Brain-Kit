...
void translate()
{
  ...
  {
    spinspeed = min(map(duty[3], 0, 100, 0, 1000), long(acc_rate * rpm) + 200);
    transpeed = map(duty[2], 0, 100, 100, -100);  //-100 to 100 and due to formula
  }
  if(angle > 180)
  {
    if(angle > 180 && angle < 250 && startimeset == false) //motor pulse at starting at 180 degrees
    {
      startime = currentime;
      dtime = 0;
      startimeset = true;
    ...
    if(dtime < duration) //is motor off time still less the motor off duration
    {
      if(reversed == true) //if spin direction is reversed
      {
        if(transpeed < 0) //if translation direction is reversed (sorry can't figure out how to clean this up)
        {
  ...
  {
    if(angle < 80 && startimeset == false) //motor pulse at 0 degrees
    {
      startime = currentime;
      dtime = 0;
      startimeset = true;
