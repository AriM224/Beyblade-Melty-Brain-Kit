...
const int NUMPIXELSB = 10; //number of leds in the bottom strip
const bool rvs_led_T = false;
const bool rvs_led_B = false;
...
void loop() 
{
  ...
  if(duty[6] > 50 && duty[3] < 20) //only reverses spin direction if throttle is under 20%
  {
    reversed = true;
  }
  else if(duty[6] < 50 && duty[3] < 20)
  {
    reversed = false;
  }
    if(rpm > 400)
    {
     ...
     if((reversed == false && duty[6] > 50) || (reversed == true && duty[6] < 50)) //activates celebration led display if reverse switch is toggled while spinning
     {
      LEDStatus = "celebrate";
     }
     else if (isAngleInRange(LEDheading, 20)) // LED turns on within ±10° of heading
     {
    ...
    {
      motorL = map(duty[2], 0, 100, -100, 100) + map(duty[1], 0, 100, -20, 20);
      motorR = map(duty[2], 0, 100, 100, -100) + map(duty[1], 0, 100, -20, 20);
    }
    else
    {
      motorL = 0;
